    ...
    public class SuriDrawingToolTile : Indicator {
		...
		protected override void OnStateChange()
		{
			if (State == State.SetDefaults)
			{
				...
				{
					typeof(ExtendedLine), typeof(DrawingTools.Line), typeof(PathTool),
					typeof(HorizontalLine), typeof(VerticalLine), 
					typeof(Rectangle), typeof(Text), typeof(Ruler),
					typeof(RectangleS), typeof(RectangleBarInfo), typeof(RectangleStrikingSpot),
				})
				{
		...
		private FrameworkElement CreateControl()
		{
			...
			while (count < elements.Count)
			{
				...
				for (int j = 0; j < NumberOfRows && count < elements.Count; j++)
				{
					...
					try
					{
						DrawingTool dt = Core.Globals.AssemblyRegistry[element.Attribute("Assembly").Value].CreateInstance(element.Name.ToString()) as DrawingTools.DrawingTool;
