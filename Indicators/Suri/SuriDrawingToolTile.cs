...
namespace NinjaTrader.NinjaScript.Indicators.Suri {
	[TypeConverter("NinjaTrader.NinjaScript.Indicators.Suri.DrawingToolIndicatorTypeConverter")]
	...
	public sealed class SuriDrawingToolTile : Indicator {
		...
		private List<XElement> SortElements(XElement elements) {
			string[] ordered =	{
				...
				typeof(RectangleS)				.FullName,
				typeof(RectangleBarInfo)		.FullName,
				typeof(SuriRuler)				.FullName,
				typeof(StrikingHighRectangle)	.FullName,
				typeof(StrikingLowRectangle)	.FullName,
			};
	...
	public sealed class DrawingToolPropertyDescriptor : PropertyDescriptor {
		...
		public	override	Type	ComponentType							{ get { return typeof (SuriDrawingToolTile); } }
		...
		public	override	object	GetValue(object component) {
			SuriDrawingToolTile c = component as SuriDrawingToolTile;
		...
		public override void SetValue(object component, object value) {
			...
			if (val && c.SelectedTypes.Element(Name) == null) {
				...
				if (c.SelectedTypes != null) {
					XElement x = c.SelectedTypes.Element(Name);
					if (x != null) x.Remove();
				}
			}
		}
	...
	public sealed class DrawingToolIndicatorTypeConverter : TypeConverter {
		...
		public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object component, Attribute[] attrs) {
			TypeConverter					tc								= component is IndicatorBase ? TypeDescriptor.GetConverter(typeof(IndicatorBase)) : TypeDescriptor.GetConverter(typeof(DrawingTool));
			...
			foreach (PropertyDescriptor pd in propertyDescriptorCollection) {
				...
				if (pd.Name == "SelectedTypes") {
					int i = 1;
					foreach (Type type in Core.Globals.AssemblyRegistry.GetDerivedTypes(typeof(DrawingTool))) {
						if (type.FullName == null) continue;
						DrawingTool tool = type.Assembly.CreateInstance(type.FullName) as DrawingTool;
