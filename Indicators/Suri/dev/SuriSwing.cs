...
namespace NinjaTrader.NinjaScript.Indicators.Suri.dev {
	public class SuriSwing : Indicator {
		...
		protected override void OnBarUpdate() {
			...
			if (saveCurrentBar != CurrentBar) {
				...
				if (lastHighCache.Count > constant) lastHighCache.RemoveAt(0);
				lastLowCache.Add(low0);
				if (lastLowCache.Count > constant) lastLowCache.RemoveAt(0);
				...
				if (lastHighCache.Count == constant) {
					...
					if (isSwingHigh) lastSwingHighValue = swingHighCandidateValue;
				...
				if (lastLowCache.Count == constant) {
					...
					if (isSwingLow) lastSwingLowValue = swingLowCandidateValue;
		...
		public int SwingLowBar(int barsAgo, int instance, int lookBackPeriod) {
			if (instance < 1)		throw new Exception(string.Format(Custom.Resource.SwingSwingLowBarInstanceGreaterEqual, GetType().Name, instance));
			if (barsAgo < 0)		throw new Exception(string.Format(Custom.Resource.SwingSwingLowBarBarsAgoGreaterEqual, GetType().Name, barsAgo));
			if (barsAgo >= Count)	throw new Exception(string.Format(Custom.Resource.SwingSwingLowBarBarsAgoOutOfRange, GetType().Name, (Count - 1), barsAgo));
			Update();
			for (int idx=CurrentBar - barsAgo - Strength; idx >= CurrentBar - barsAgo - Strength - lookBackPeriod; idx--) {
				if (idx < 0) return -1;
				if (idx >= swingLowSwings.Count) continue;
				if (swingLowSwings.GetValueAt(idx).Equals(0.0)) continue;
		...
		public int SwingHighBar(int barsAgo, int instance, int lookBackPeriod) {
			if (instance < 1)		throw new Exception(string.Format(Custom.Resource.SwingSwingHighBarInstanceGreaterEqual, GetType().Name, instance));
			if (barsAgo < 0)		throw new Exception(string.Format(Custom.Resource.SwingSwingHighBarBarsAgoGreaterEqual, GetType().Name, barsAgo));
			if (barsAgo >= Count)	throw new Exception(string.Format(Custom.Resource.SwingSwingHighBarBarsAgoOutOfRange, GetType().Name, (Count - 1), barsAgo));
			Update();
			for (int idx=CurrentBar - barsAgo - Strength; idx >= CurrentBar - barsAgo - Strength - lookBackPeriod; idx--) {
				if (idx < 0) return -1;
				if (idx >= swingHighSwings.Count) continue;
				if (swingHighSwings.GetValueAt(idx).Equals(0.0)) continue;
				if (instance <= 1) // 1-based, < to be save
					return CurrentBar - idx;
				instance--;
			}
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private Suri.dev.SuriSwing[] cacheSuriSwing;
		public Suri.dev.SuriSwing SuriSwing(int strength)
		{
			return SuriSwing(Input, strength);
		}

		public Suri.dev.SuriSwing SuriSwing(ISeries<double> input, int strength)
		{
			if (cacheSuriSwing != null)
				for (int idx = 0; idx < cacheSuriSwing.Length; idx++)
					if (cacheSuriSwing[idx] != null && cacheSuriSwing[idx].Strength == strength && cacheSuriSwing[idx].EqualsInput(input))
						return cacheSuriSwing[idx];
			return CacheIndicator<Suri.dev.SuriSwing>(new Suri.dev.SuriSwing(){ Strength = strength }, input, ref cacheSuriSwing);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.Suri.dev.SuriSwing SuriSwing(int strength)
		{
			return indicator.SuriSwing(Input, strength);
		}

		public Indicators.Suri.dev.SuriSwing SuriSwing(ISeries<double> input , int strength)
		{
			return indicator.SuriSwing(input, strength);
		}
	}
